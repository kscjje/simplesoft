...
$.excel = {
		createEvent: function(_file) {		
			$(_file).off().change(function() {
		        ...
		        if(window.FileReader){ 
		            // modern browser 
		        	if($(this)[0].files[0] != null) filename = $(this)[0].files[0].name;
		        } else { 
		            // old IE 
		        ...
		        if(filename != null && filename != "")
		        {
					...
					if(ext.toLowerCase() != 'xlsx' || ext.toLowerCase() != 'xlsx') {
						...
						$(_file).val("");
						filename="";
						$.alert("확장자가 xlsx, xls인 엑셀파일만 업로드 가능합니다.", "실패");
						return false;
					}
		        }else return false;
			});
		},
		openClickFunction: function(event, id) {
			$("#"+event).on("click", function () {
				var excelFormId = "excelForm";
				if(id != null && id != "") excelFormId = id;
				$.excel.open(excelFormId);
			});
		},
		open: function(excelFormId) {
			const layer = $("#"+excelFormId);
		...
		downloadClickFunction: function(obj, event, url, fnName) {
			$("#"+event).on("click", function () {
				...
				// TODO :: 확인 창 띄워주기 => admin, sample도 적용
				$.excel.download(url, fnName);
			});
		},
		download: function(url, fnName) {
			
			var strParam = "";
			if(fnName != null && fnName != "") strParam = eval(fnName)();
			
			$.showMask();
			
			$.fileDownload(url+"&" + strParam, {
		        successCallback: function (link) {
		        	var result = "";
		        	var nameEQ = "fileDownload" + "=";
		    	    ...
		    	    if(result.indexOf("false") > -1) {
		    	    	$.alert("다운로드 실패 :: 파일이 없거나 일시적 오류 입니다.", "실패");
		    	    	location.reload();
		    	    }
		...
		submitClickFunction: function(event, id, url, fnName) {
			$("#"+event).on("click", function () {
				var excelFormId = "excelForm";
				if(id != null && id != "") excelFormId = id;
				$.excel.submit(excelFormId, url, fnName);
			});
		},
		submit: function(id, url, fnName) {
			const layer = $("#"+id);
			const file = layer.find("input:file");
			
			if(file[0].files[0] == null || file[0].files[0] == "")
			{
				$.alert("파일을 선택해 주세요.", "실패");
				return false;
			}
	        ...
	        if(fnName != null && fnName != "")
	        {
	        	...
	        	if(jsonResult != null && jsonResult != "")
	        	{
	        		...
	        		{
	            		var key = keys[i];
	            		formData.append(key, jsonResult[key]);            		
	        		}
	        	}            	
	        }
	        ...
	        $.ajax({
	            ...
	            success : function(data) {
	            	var data = data.data;
	            	if(data["resultCode"] == "SUCCESS") $.alert("정상적으로 업로드 되었습니다.", "성공");
	            	else $.alert(data["resultMessage"], "실패");
	            	
	            	location.reload();
	            	$.hideMask();
	            },
		        error: function (xhr, ajaxOptions, thrownError) {
					$.alert("엑셀업로드 처리시 오류가 발생하였습니다.", "실패");
					location.reload();
					$.hideMask();
					return;
		        }
		...
		project: {
			submitClickFunction: function(event, id, url, fnName) {
				$("#"+event).on("click", function () {
					var excelFormId = "excelForm";
					if(id != null && id != "") excelFormId = id;
					$.excel.project.submit(excelFormId, url, fnName);
				});
			},
			submit: function(id, url, fnName) {
				const layer = $("#"+id);
