...
public class DateUtil {
    ...
    static {
        try {
          ...
          LOGGER.error("DateUtil Time Zone 설정 중 오류발생", e);
        }
      }
    ...
    public static String convertDate(String strSource, String fromDateFormat, String toDateFormat, String strTimeZone) {
        ...
        Date date = null;
        String tmpFromDateFormat = "";
        String tmpToDateFormat = "";

        if (StringUtils.isNullToString(strSource).trim().equals("")) {
            return "";
        }
        if (StringUtils.isNullToString(fromDateFormat).trim().equals(""))
            tmpFromDateFormat = "yyyyMMddHHmmss"; // default값
        if (StringUtils.isNullToString(toDateFormat).trim().equals(""))
            tmpToDateFormat = "yyyy-MM-dd HH:mm:ss"; // default값

        try {
            simpledateformat = new SimpleDateFormat(tmpFromDateFormat, Locale.getDefault());
            ...
            simpledateformat = new SimpleDateFormat(tmpToDateFormat, Locale.getDefault());
    ...
    * @return 변환된 문자열
    ...
    public static String validChkDate(String dateStr) {
        ...
        if (dateStr.length() == 10) {
            tmpDateStr = StringUtils.removeMinusChar(dateStr);
        }
        return tmpDateStr;
    }
    ...
    public static String validChkTime(String timeStr) {
        String tmpTimeStr = timeStr;

        if (tmpTimeStr.length() == 5) {
            tmpTimeStr = StringUtils.remove(tmpTimeStr, ':');
        }
        if (tmpTimeStr == null || !(tmpTimeStr.trim().length() == 4)) {
            throw new IllegalArgumentException("Invalid time format: " + tmpTimeStr);
        }
    ...
    public static Set<LocalDate> getDatesBetween(LocalDate startDate, LocalDate endDate) {
        Set<LocalDate> resultList = startDate.datesUntil(endDate).collect(Collectors.toSet());
